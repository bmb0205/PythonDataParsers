...
* Infile(s) [TTD files in .txt format, 2015 versions used]:
...
def parseTargetDisease(tempPath, nodeSet):
	""" Parses target-disease_TTTD2016.txt for id, target name and indication and writes relationships """
		...
		if opt == "-h":
			...
			for root, dirs, files in os.walk(topDir):
				...
				if root.endswith("TTD"):
					...
					print "\n%s total Therapeutic Target Database nodes have been created." %locale.format('%d', (nodeCount + nodeCount2), True)
					print "\n%s total Therapeutic Target Database relationships have been created." %locale.format('%d', (KEGGRelnCount + wikiRelnCount), True)
